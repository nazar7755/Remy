...
import { itemMatchesTypeFilter } from '../lib/memoriesFilters'
...
import { MemoryTypeFilterBar } from './MemoryTypeFilter'
...
export function MemoriesPage({
  ...
  const [typeFilter, setTypeFilter] = useState<MemoriesTypeFilter>('All')
  ...
  }, [items, typeFilter, effectiveQuery, sort])
  ...
  return (
    <section className="space-y-6">
      <SearchBar
        ...
        placeholder="Search memories by name, path, type, source, or text…"
        className="max-w-xl"
      />
      ...
      <div className="flex flex-wrap items-center gap-2">
        <MemoryTypeFilterBar value={typeFilter} onChange={setTypeFilter} />
      </div>
      ...
      <div className="flex items-start gap-5">
        <div className="min-w-0 flex-1">
          {loading && items.length === 0 ? (
            <p className="text-sm text-remy-muted">Loading memories…</p>
          ) : displayed.length === 0 ? (
            <p className="text-sm text-remy-muted">
              {effectiveQuery || typeFilter !== 'All'
          ...
          <p className="mt-6 text-xs text-remy-muted">
            {displayed.length} of {items.length} memor
